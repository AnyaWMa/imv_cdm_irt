...
cdm.sim<-function(a,N=500,sk.offset=0,nsk=6,bound=NULL,modeltype="DINA") {
  ##skills
  th<-rnorm(N)
  sk<-runif(nsk)
  ...
  ##cv imv values
  om<-oos.compare.newresp(resp,qm,truep=p.true, modeltype)
  ##
  om
}
...
a<-sort(runif(100,min=0,max=3))
for (i in c(.1,.2,.3)) out_dina_offset15[[as.character(i)]]<-mclapply(a,cdm.sim,mc.cores=10,sk.offset=1.5,bound=i, modeltype = "DINA")
out_dina_offset15[["a"]] <- a
save(out_dina_offset15 , file = "../simulation_data/out_dina_offset15.RData")

out_dina_offset05 <-list()
a<-sort(runif(100,min=0,max=3))
...
a<-sort(runif(100,min=0,max=3))
for (i in c(.1,.2,.3)) out_dino_offset15[[as.character(i)]]<-mclapply(a,cdm.sim,mc.cores=10,sk.offset=1.5,bound=i, modeltype = "DINO")
out_dino_offset15[["a"]] <- a
save(out_dino_offset15 , file = "../simulation_data/out_dino_offset15.RData")

out_dino_offset05 <-list()
a<-sort(runif(100,min=0,max=3))
for (i in c(.1,.2,.3)) out_dino_offset05[[as.character(i)]]<-mclapply(a,cdm.sim,mc.cores=10,sk.offset=0.5,bound=i, modeltype = "DINO")
out_dino_offset05[["a"]] <- a
save(out_dino_offset05 , file = "../simulation_data/out_dino_offset05.RData")

######
pdf("../plots/simulation1_sk15_n500_gdina.pdf")
