...
mp <- personparm(cdmfit,what = "mp")
attpat <- attributepattern(K)                  # 2^K x K
pmp <- matrix(NA, N, J)

p.cdm<-cdm.pr.marg.update(sim$dat,Q, "DINA", "mp")


cdmfit <- GDINA(sim$dat,Q, model = "DINA", mono.constraint = TRUE)
...
for (j in 1:J) {
  pj_class <- cdmfit$LC.prob[j, ]              # length 2^K: item prob for each class
  for (i in 1:N) {
    pmp[i, j] <- sum(mp[i, ] * pj_class)       # Expected item response prob for item j
  }
}
